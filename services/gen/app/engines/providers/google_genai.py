...
★승격 완료 (R5) — **라이브 엔진의 어댑터다** (05 부록 A-17 ① · A-18 ⑦).
`research/genlab/providers/google_genai.py`의 복사본이며 import는 **stdlib +
google-genai SDK만** (06 §1 N5). `.base`는 함께 승격된 짝이다.

⚠ 이 모듈은 최상단에서 SDK를 import한다. 따라서 **`ENGINE=mock` 경로에서 절대
import되면 안 된다** — `engines/__init__.py`의 빌더 함수 안에서만 도달하게 해 두었다
(06 §13 "SDK 없이 부팅"). `engines/providers/__init__.py`가 비어 있는 이유도 같다.
