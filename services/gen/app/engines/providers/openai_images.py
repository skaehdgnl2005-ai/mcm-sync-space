...
★승격 완료 (R5) — **폴백 엔진의 어댑터다** (05 부록 A-16 ④ · A-17 ①: 사전 생성 =
`gpt_image`, 라이브가 흔들리면 env 한 줄로 전환). `research/genlab/providers/
openai_images.py`의 복사본이며 import는 **stdlib + openai SDK만** (06 §1 N5).

⚠ google_genai.py와 같은 이유로 이 모듈도 **`ENGINE=mock` 경로에서 import되면 안 된다.**
